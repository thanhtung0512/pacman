        ...
        for action in [
            ...
            if not hitsWall:
                ...
                for corner in state[1]:
                    # If the resulting position doesnt reach a corner
                    # add the corner to the list of remaining corners
                    if corner != next_position:
                        remaining_corners.append(corner)

                # Successor has the next position, the remaining corners,
                # the action and the cost
                successors.append(
                    ((next_position, tuple(remaining_corners)), action, 1)
                )
...
def cornersHeuristic(state: Any, problem: CornersProblem) -> int:
    ...
    # Unpack the current state and the tuple of unvisited corners
    current_node, unvisited_corners = state

    # Initialize the heuristic cost
    heuristic_cost = 0

    # Loop while there is at least one unvisited corner
    while len(unvisited_corners) != 0:
        # Create a dictionary to store the distances
        distances = {}

        # Calculate the Manhattan distance from the current node to each unvisited corner
        ...
        closest_corner, min_distance = min(distances.items(), key=lambda data: data[1])

        # Add the minimum distance to the heuristic cost
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        # Check if the current position coincides with the nearest food
        nearest_food = min(
